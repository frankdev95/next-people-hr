...
import addEmployeeForms from "../../utils/forms/add-employee/add-employee-form";
import Stage from "../ui/Form/Stage.js/Stage";
import Close from "../ui/Button/Close";
import FormCompleted from "./FormCompleted";
import Modal from "../ui/Modal/Modal";
import ConfirmationMessage from "../ui/ConfirmationMessage.js/ConfirmationMessage";
...
const AddEmployeeForm = (props) => {
  ...
  const {
    ...
    setForm,
    setInputsAsTouched,
  } = useForm(addEmployeeForms[currentForm]);

  const { onModalClose: closeModal } = props;

  const onModalClose = () =>
    currentForm < addEmployeeForms.length
      ? setCloseConfirmation(true)
      : closeModal();

  useEffect(() => {
    if (currentForm < formStages.length - 1) {
      const form = addEmployeeForms[currentForm];
      ...
      setForm(addEmployeeForms[currentForm]);
    } else {
      const formValues = [];
    }
  ...
  return (
    <div className={styles.container}>
      ...
      {closeConfirmation && (
        <Modal styles={{ borderRadius: "10px" }}>
          <ConfirmationMessage
            ...
            message="Are you sure you want to leave before form submittion? Your data will be lost!"
          />
        </Modal>
      )}
      <h1>New Employee</h1>
